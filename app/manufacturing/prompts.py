...
def get_vlm_descriptive_prompt(bom_context: str = "", rag_context: str = "", system_anchors: Optional[List[str]] = None) -> str:
    ...
    confidence_rules = (
        ...
        "STOP after Section 4. Do NOT add Section 5 or any continuation.\n\n"
    )

    # ── Vocabulary: split into 2D (all sections) vs 3D (Section 4 only) ─────
    vocab_block = (
        "VOCABULARY RULES:\n"
        "  2D PROFILE TERMS (use in Sections 1\u20133 for individual views):\n"
        "    Outlines : Rectangular outline | Circular outline | L-shaped profile | U-shaped profile | Z-shaped profile | T-shaped profile | Trapezoidal profile\n"
        ...
        "  3D SHAPE TERMS (ONLY allowed in Section 4 — 3D reconstruction):\n"
        "    Flat Plate | Rectangular Base | L-shaped Bracket | U-shaped Channel | Z-shaped Bracket | Hat Channel | Box | Cylinder\n"
        "  Confidence tag: <green>word</green> = clearly visible | <orange>word</orange> = partially visible | <red>word</red> = inferred\n"
        "CRITICAL: Using a 3D shape term (Box, Cylinder, Bracket, etc.) inside Section 1, 2, or 3 is a CRITICAL ERROR.\n\n"
    )
    ...
    return (
        ...
        "If an Isometric (ISO) or 3D view is provided, use it ONLY in Section 4 as additional reference — it does NOT replace orthographic views.\n\n"
        ...
        "### 1. BOM FACT CHECK\n"
        "Summarize what the BOM / engineer notes state (materials, processes, part names). "
        "If no BOM provided, write: 'No BOM provided.'\n\n"
        "### 2. 2D VIEW-BY-VIEW PROFILES\n"
        "For EACH available view, describe ONLY the flat 2D outline and visible features. "
        "Use 2D PROFILE TERMS only. DO NOT use any 3D shape term here.\n"
        "- Top View: [2D outline + visible holes/features]\n"
        "- Front View: [2D outline + visible holes/features]\n"
        "- Side View (if present): [2D outline + visible holes/features]\n"
        "- ISO/3D View (if present): Skip detailed description here — reserve for Section 4.\n\n"
        "### 3. SYMBOL & TEXT SEARCH\n"
        "Answer True or False for each category. If True, state location briefly.\n"
        "- Weld symbol detected: True/False\n"
        "- Surface finish mark detected: True/False\n"
        "- Other text annotation found (describe briefly, no numbers):\n\n"
        "### 4. 3D RECONSTRUCTION\n"
        "NOW combine all 2D profiles from Section 2 + symbol evidence from Section 3 + BOM facts from Section 1 "
        "to infer the final 3D shape. Use 3D SHAPE TERMS here. "
        "Template: This is a {3D shape} consisting of {main part} with {secondary features}. "
        "It likely requires {key processes based on symbols/BOM}.\n"
        "If ISO view was provided, use it here to confirm or refine your 3D inference.\n"
    )
