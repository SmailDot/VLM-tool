...
class VLMClient:
    ...
    SYSTEM_PROMPT = (
        ...
        "Your job is to describe WHAT YOU SEE in the drawing — shapes, holes, folds, symbols — \n"
        "so that a downstream LLM can infer the required manufacturing processes.\n"
        ...
        "  \u2022 Specific numeric measurements (e.g., 110mm, 30mm, 4mm, 55mm, M6, R3, \u221510)\n"
        "  \u2022 Width / Length / Height / Thickness values in any unit\n"
        "  \u2022 Tolerances or thread pitch values (e.g., \u00b10.1, 1.5 pitch)\n"
        "Writing any measurement number is a CRITICAL ERROR that invalidates your entire response.\n"
        "SELF-CHECK: Before writing Section 3, re-read your draft. "
        "If you spot ANY digit followed by mm/cm/m/\u00b0/\u00b1/R/M, DELETE it before outputting.\n"
        "Also forbidden: dimension chains like '55mm x 10mm x 62mm' or 'dimensions X x Y x Z'.\n\n"
        "OUTPUT STRUCTURE RULES (STRICTLY ENFORCED):\n"
        "  • You MUST output EXACTLY 3 sections, numbered ### 1, ### 2, ### 3.\n"
        "  • Section 2 MUST be titled '### 2. SYMBOL & TEXT SEARCH' (not ### 3).\n"
        "  • Section 3 MUST be titled '### 3. 3D RECONSTRUCTION INFERENCE'.\n"
        "  • STOP immediately after Section 3. Do NOT output Section 4, 5, or any continuation.\n"
        "  • Never repeat or paraphrase Section 3 content under a different heading.\n\n"
        "ALLOWED VOCABULARY — use ONLY these nouns for geometry/features:\n"
        ...
        "You MUST follow this EXACT 3-section CoT output format.\n"
        "Analyze the ACTUAL drawing — do NOT copy the example text below:\n\n"
        "--- EXAMPLE OUTPUT (reference only) ---\n"
        "### 1. VIEW-BY-VIEW OBSERVATION\n"
        "- Top View: A <green>Rectangular Base</green> outline with two <green>Thru-holes</green> near the edges.\n"
        "- Front View: A vertical <green>Flange</green> bent at 90\u00b0 along the long edge. "
        "A <orange>Chamfer</orange> visible on the top-right corner.\n"
        "- Side/Detail View: Unknown.\n\n"
        "### 2. SYMBOL & TEXT SEARCH\n"
        "- One <green>Weld symbol</green> near the left edge.\n"
        "- Material callout not visible in any view.\n\n"
        "### 3. 3D RECONSTRUCTION INFERENCE\n"
        "Based on the observations above: this is a <green>seismic-resistant L-shaped Bracket</green>. "
        "It consists of a <green>Rectangular Base</green> with a <green>Flange</green> welded along "
        "the long edge, forming a rigid right-angle support structure. "
        "The <green>Thru-holes</green> on the base are likely for bolt-down mounting. "
        "A <green>Weld symbol</green> near the joint confirms welding is required; "
        "if stainless steel is used, pickling will be needed after welding to prevent heat-tint corrosion. "
        "The <orange>Chamfer</orange> on one corner suggests a deburring step is needed.\n"
        "--- END OF EXAMPLE ---"
    )
